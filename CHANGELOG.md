...
## 0.5.0
Fewer tokens across **every** model in the pipeline (lead, workers, and the CLI reviewers), with the quality gates kept intact. The savings come from cutting waste, never rigor: risky work still gets the full crew and a full review.

- **Size the work first.** A cheap first pass rates the task and the pipeline runs only as deep as it needs: Small (one file, few lines, no auth/money/DB risk) → the chair edits directly, then one review; Medium → scout + one coder + tester + one review; Large / risky → full crew, strict, up to `max_rounds`. Before this, every task ran the full five-agent crew regardless of size — that was the biggest source of wasted tokens. Every code change still gets **at least one independent review**; sizing removes ceremony, not the review.
- **Ponytail — the smallest solution that works.** The lead's plan and the coder's implementation now climb the Ponytail 7-rung ladder (need it at all? → already in this codebase? → stdlib → native platform → already-installed dependency → one line → minimum viable) and stop at the lowest rung that fully solves the task, asking "what is the simpler way, and why can't it be used?" first. Minimal code is fewer tokens to generate and read at every hop, and clearer to maintain. The ladder follows the Ponytail methodology by Dietrich Gebert (github.com/DietrichGebert/ponytail); `crew/SKILL.md` credits it and points to the full plugin for coding beyond the crew. This is the core idea, not his full ruleset, so it moves in the same direction without claiming his measured numbers.
- **Context diet.** The scout returns paths, `function:line` and one line per item — never whole files; later steps read `state.md` instead of re-opening unchanged files; worker prompts get only the slice they need, not the full scout report; the reviewer still gets a scoped diff of just the changed files. Every model in the chain reads less.
- **Stop on PASS.** A passing review ends the loop immediately, whatever rounds remain; Small/Medium tasks start at one round. Only a FAIL spends another round.
- **Leaner auto-mode hook.** The `UserPromptSubmit` hook fired ~120 words of reminder into **every** message, coding or not. Trimmed to ~40 that point at the skill's own Auto-mode section, so the per-turn overhead is a third of what it was.
