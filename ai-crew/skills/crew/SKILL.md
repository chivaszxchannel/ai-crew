...
2. Run the reviewer chain script, passing the CLI reviewers from config in order:
   ...
   Exit 0 = PASS, 1 = FAIL (findings in the printed file), 3 = no CLI reviewer could run; the first output line says which reviewer was used or why each was skipped (`REVIEWER_NOT_FOUND` / `_NOT_LOGGED_IN` / `_RATE_LIMITED` / `_ERROR`). `REVIEWER_USED: <name> (no VERDICT line - output shown verbatim)` means the reviewer answered but did not follow the format: read what it actually said, treat it as FAIL, and say in the report that the verdict line was missing.
